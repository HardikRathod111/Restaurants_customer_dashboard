...
const Managemenu = () => {
    ...
    const openEditModal = async (id) => {
        try {
          const response = await fetch(`https://restaurants-customer-dashboard.vercel.app/api/v1/manageorder/items/${id}`); // Adjust endpoint
          const data = await response.json();
    ...
    const updateItem = async (updatedItem) => {
        try {
          ...
          const response = await fetch(`https://restaurants-customer-dashboard.vercel.app/api/v1/manageorder/editItem/${updatedItem._id}`, {
            method: 'PUT',
            body: formData,
          });
      ...
      const handleOpenDelete = (item) => {
        ...
        const handleDelete = async (itemId) => {
            ...
            try {
                const response = await axios.delete(`https://restaurants-customer-dashboard.vercel.app/api/v1/manageorder/deleteItem/${itemId}`);
    ...
    const handleAddCategory = async () => {
        ...
        try {
            const response = await fetch('https://restaurants-customer-dashboard.vercel.app/api/v1/category/createCategory', {
                method: 'POST',
                body: formData,
            });
                    ...
                    {items.length > 0 ? (
                        items.map((item, index) => (
                            <div key={item._id} className="bg-gray-800 w-full  rounded-lg p-4 text-gray-300 relative">
                                ...
                                <img src={`https://restaurants-customer-dashboard.vercel.app/${item.imageUrl}`} alt={item.itemName} className="w-40 ml-1 h-28  object-cover rounded-md mb-2" />
                                </div>
