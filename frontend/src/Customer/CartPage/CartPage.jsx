...
export default function CartPage({ cartItems }) {
  ...
  const removeItem = async (orderId) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/v1/addCart/deleteItem/${orderId}`, // Ensure backend endpoint matches this URL
        {
          method: "DELETE",
          headers: {
  ...
  const handlePayClick = (paymentMethod) => {
    ...
    axios
      .post('http://localhost:8080/api/v1/order/create', orderData)
      .then((response) => {
            ...
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-slate-800 rounded-md flex items-center justify-center">
                <span role="img" aria-label="burger" className="text-xl">
                  <img
                    src={`http://localhost:8080/${item.itemId.imageUrl}`}
